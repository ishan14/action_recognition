...
class SlowFast(nn.Module):
    def __init__(self, block=Bottleneck, layers=[3, 4, 6, 3], class_num=10, dropout=0.5 ):
        ...
        self.fast_conv1 = nn.Conv3d(18, 8, kernel_size=(5, 7, 7), stride=(1, 2, 2), padding=(2, 3, 3), bias=False)
    ...
    def forward(self, input_fast, input_slow):
        fast, lateral = self.FastPath(input_fast[:, :, ::2, :, :])
        slow = self.SlowPath(input_slow[:, :, ::16, :, :], lateral)
    ...
    def _make_layer_fast(self, block, planes, blocks, stride=1, head_conv=1):
        ...
        # if stride != 1 or self.fast_inplanes != planes * 4:
            ...
            # downsample = nn.Sequential(nn.Conv3d(self.fast_inplanes,planes * 4,kernel_size=1,stride=(1,stride,stride),bias=False), nn.BatchNorm3d(planes * 4))


        layers = []
        layers.append(block(self.fast_inplanes, planes, stride, downsample, head_conv=head_conv))
        self.fast_inplanes = planes * block.expansion
        # self.fast_inplanes = planes * 4
    ...
    def _make_layer_slow(self, block, planes, blocks, stride=1, head_conv=1):
        ...
        if stride != 1 or self.slow_inplanes != planes * 4:
            ...
            downsample = nn.Sequential(nn.Conv3d(self.slow_inplanes,planes * 4,kernel_size=1,stride=(1,stride,stride),bias=False), nn.BatchNorm3d(planes * 4))

        layers = []
        layers.append(block(self.slow_inplanes, planes, stride, downsample, head_conv=head_conv))
        # self.slow_inplanes = planes * block.expansion
        self.slow_inplanes = planes * 4
        for i in range(1, blocks):
            layers.append(block(self.slow_inplanes, planes, head_conv=head_conv))
  
        # self.slow_inplanes = planes * block.expansion + planes * block.expansion//8*2
        self.slow_inplanes = planes * 4 + planes * 4//8*2
        #self.slow_inplanes = planes * block.expansion
        return nn.Sequential(*layers)
